...
function draw() {
  ...
  for (var y = 0; y < rows; y++) {
    var xoff = 0;
    for (var x = 0; x < cols; x++) {
      terrain[x][y] = map(noise(xoff, yoff), 0, 1, -2000, 1000);
      xoff += 0.02;
    }
    yoff += 0.02;
  }
  ...
  for (var y = 0; y < rows - 1; y++) {
    ...
    for (var x = 0; x < cols; x++) {
      stroke(0, 0, map(count, 0, 50, 100, 0));//map(count, 0, 50, 1, 0));
...
function sun() {
  ...
  sun = random(height / 4);
  //fill(180, 50, 50);
  rect(width - 1500, height/9, 500,3000, 2000);
//   for (let i = 0; i < 5; i++) {
//   push();
//   rotateX(PI/2)
//   ellipse(width - random(100, 500), height / 8, 500, 500);
//   pop();
// }
